...
function __initLog(){log=(function(){var getTree=function(data){var toStr=Object.prototype.toString,_=arguments,json=[],x=[],type=toStr.call(data).replace(/[\[\] ]|object/ig,'').toLowerCase()||(data?'object':data===null?'null':'undefined'),br='\n',iv=_[2]==void(0)?'':_[2]+'    ';var k=iv+(_[1]!==void(0)?'"'+_[1]+'":':'');if(iv.length>36){return json.concat(k+'[\u9012\u5f52\u6ea2\u51fa]').join('')}if(type=='array'){var N=data.length;json.push(k+'['+(N?br:''));for(var i=0;i<N;i++){x.push(getTree(data[i],void(0),iv))}json.push(x.join(','+br),(N?br+iv:'')+']')}else if(type=='object'&&data instanceof Object){var N=0;for(var u in data){N++}json.push(k+'{'+(N?br:''));for(var u in data){if(u.indexOf('__')!=0&&data.hasOwnProperty(u)){var sub=data[u]&&data[u].Type?''+data[u]:data[u];x.push(getTree(sub,u,iv))}}json.push(x.join(','+br),(N?br+iv:'')+'}')}else{data=data===void(0)||data===null?data:(type=='object'&&!(data instanceof Object))?'[Native object]':(type=='function'?'@Func':data.Type?(data+''):data);json.push(k+(type=='string'?'"'+data+'"':data))}return json.join('')};return(function(html,q,w,o,old,b,timer){return function(){if(!__initLog.debug_mode){return}if(!w){w=Yobj.createNode('textarea',document.body);o=w.one();old=o.style.cssText='width:320px;height: 480px;overflow: auto;right: 0;bottom: 0;position: '+(Yobj.ie==6?'absolute':'fixed')+';border:3px solid #7F9DB9;border-right:none;border-bottom:none;background:#000;color:#ccc;font: 12px/1.2 verdana;z-index:99999999';w.keydown(function(e){if(e.keyCode===27||e.keyCode===18){w.hide()}else if(e.shiftKey){if(b=!b){w.setStyle('width:99%;height:'+(Yobj.ie==6?(Yobj.getSize().offsetHeight-9)+'px':'100%'))}else{o.style.cssText=old}}})}var data=[],args=[].slice.call(arguments);for(var i=0,j=args.length;i<j;i++){data.push(getTree(args[i]))}html.push((++q)+' >\n'+data.join('\n')+'\n\n');clearTimeout(timer);timer=setTimeout(function(){o.value+=html.join('');html=[];o.scrollTop=o.scrollHeight;w.show()},100)}})([],0)})()};
(function() {
	...
	__Err = !!(location.href+'').match(/\?~debug/i),
	...
	__initLog.debug_mode = !!(location.href+'').match(/~\w+/i);
	...
	__C.mix(Function.prototype, {
		proxy: function(proxy) {
			...
			})(this, Slice.call(arguments, 1))// ff setTimeout has a param
		},
	...
	__C.mix(__C, {
		...
		dejson: function(data) {
			try {
				...
				__Err && log(this.Type + '.dejson() - parse Error: ', data)
			}
		},
		...
		$: function(selector, contains, one) {
			...
			nodes.length === 0 && arguments.length && __Err && log(this.Type + '.get find"' + selector + '" is empty');
			return nodes
		},
		get: function() {
		...
		_isMatchAttr: function(el, key, vs, val) {
			var attr = el.getAttribute(key) || '', prop = el[key] || '';
			switch (vs) {
			case '':
				return !! attr;
			case '=':
		...
		one: function(selector, contains, clearError) {
			return  this.$(selector, contains, true)[0] || null;
		},
		...
		fx: function(fn, config) {
			...
			if (!config.init || false !== config.init.call(Y)) {
				if (mutex && mutex in this.__FxMutexs) {
					if (over || over === void 0) {
						this.endFx(this.__FxMutexs[mutex])
					} else {
		...
		postMsg: function(type) {
			return (function(Y, center, er, args, msg, queue, reValue, guid, o) {
                ...
                __Err && log('sender='+Y.Type+'\nmassage = '+type+'\nparam = '+args.join('\n')+'\nreturn obj = '+ (msg.from || '-')+'\nback data = '
                    + (msg.data || '-')+'\nread count = '+ er.length);
				return Y.mix(msg, {
					length: er.length
				})
	...
	var Class = function(classPath, props) {
        ...
        name = link[1] || 'None';
        if (name in Class.lib) {
            return Class.lib[name]// same name class
        }
    ...
    (function(scripts, __Sysini, __readOnly, coreScript) {
        for (var i = 0, j = scripts.length; i < j; i++) {
            ...
            if (/(^|\/)yclass[^\/]*\.js/i.test(src)) {
                __Sysini.CorePath =src.replace(/[^\/]+\.js(\?.*)?$/i, '');
            }
        }		
